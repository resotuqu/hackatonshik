...
window.createAvatarCropperModal = function createAvatarCropperModal(config) {
    const i18n = window.__cropperI18n ?? {};
    const t = (key, fallback) =>
        typeof i18n[key] === "string" ? i18n[key] : fallback;
    ...
    return {
        ...
        advanceBatch() {
            while (this.batchCursor < this.batch.length) {
                ...
                this.modalCaption = t(
                    "batch_caption",
                    `File ${this.batchCursor + 1} of ${this.batch.length}`,
                )
                    .replace(":current", String(this.batchCursor + 1))
                    .replace(":total", String(this.batch.length));
                this.startCropForFile(f);
                return;
            }
...
document.addEventListener('alpine:init', () => {
    window.Alpine.data('avatarCropper', (config) => window.createAvatarCropperModal(config));
});
