...
@section('slot')
    <div class="mx-auto w-full max-w-md space-y-4" x-data="{ recovery: false }">
        <x-mary-card title="{{ __('ui.auth.two_factor.card_title') }}" class="card border border-base-300 bg-base-100">
            <p class="text-sm text-base-content/70" x-show="!recovery">
                {{ __('ui.auth.two_factor.description_code') }}
            </p>
            <p class="text-sm text-base-content/70" x-cloak x-show="recovery">
            ...
            <button type="button" class="link link-primary mt-4 text-xs"
                ...
                x-bind:x-text="recovery ? '{{ __('ui.auth.two_factor.use_app') }}' : '{{ __('ui.auth.two_factor.use_recovery') }}'">
                {{ __('ui.auth.two_factor.use_recovery') }}
            </button>
        </x-mary-card>
    </div>
@endsection
