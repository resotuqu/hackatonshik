...
<div class="space-y-3">
    ...
    <div class="grid gap-2">
        @if (request()->isSecure())
            {{-- Яндекс ID SDK (мгновенная авторизация, только HTTPS) --}}
            <div id="yandex-btn-{{ $mode }}" class="w-full"></div>
        @else
            <a href="{{ route('auth.yandex.redirect') }}"
               class="group flex w-full items-center justify-center gap-2.5 rounded-xl border border-base-300 bg-base-100 px-4 py-2.5 text-sm font-semibold text-base-content transition hover:border-[#FC3F1D]/30 hover:bg-[#FC3F1D]/5 focus:outline-none focus:ring-2 focus:ring-[#FC3F1D]/30">
                ...
                {{ $mode === 'login' ? __('ui.auth.login.yandex') : __('ui.auth.register.yandex') }}
            </a>
        @endif

        {{-- VK ID --}}
        <a href="{{ route('auth.vk.redirect') }}"
           class="group flex w-full items-center justify-center gap-2.5 rounded-xl border border-base-300 bg-base-100 px-4 py-2.5 text-sm font-semibold text-base-content transition hover:border-[#2787F5]/30 hover:bg-[#2787F5]/5 focus:outline-none focus:ring-2 focus:ring-[#2787F5]/30">
            ...
            {{ $mode === 'login' ? __('ui.auth.login.vk') : __('ui.auth.register.vk') }}
        </a>
    </div>
</div>

@if (request()->isSecure())
    <form id="ya-form-{{ $mode }}" action="{{ route('auth.yandex.token') }}" method="POST" class="hidden">
        @csrf
        ...
        window.YaAuthSuggest.init(
            {
                ...
                response_type: 'token',
                redirect_uri: '{{ route('auth.yandex.token-page') }}'
            },
            '{{ rtrim(url('/'), '/') }}',
            {
                view: 'button',
