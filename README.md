...
Используется Pest 4 с `RefreshDatabase` для фич-тестов. End-to-end в браузере: PHPUnit + [Laravel Dusk](https://laravel.com/docs/dusk) в `tests/Browser`, запуск `php artisan dusk` (локально нужны Chrome/ChromeDriver и отдельная БД для Dusk — см. `.env.example` / `database/dusk.sqlite`).
